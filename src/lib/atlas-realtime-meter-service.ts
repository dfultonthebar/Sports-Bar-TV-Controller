...
export class AtlasRealtimeMeterService extends EventEmitter {
  ...
  startListening(): void {
    if (this.isListening) {
      console.log('Already listening for meter updates')
      return
    }
    ...
    this.udpSocket.on('message', (msg, rinfo) => {
      try {
        ...
        console.error('Error processing UDP meter data:', error)
      }
    })
    ...
    this.udpSocket.bind(this.udpPort, () => {
      ...
      console.log(`Listening for Atlas meter updates on UDP port ${this.udpPort}`)
      this.emit('listening')
    })
  }
  ...
  stopListening(): void {
    if (this.udpSocket) {
      ...
      console.log('Stopped listening for meter updates')
      this.emit('stopped')
    }
  }
