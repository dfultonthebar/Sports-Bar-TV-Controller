...
export class CableBoxCECService {
  ...
  async discoverAdapters(): Promise<{ devicePath: string; info: string }[]> {
    try {
      ...
      console.error('Error discovering CEC adapters:', error.message)
      return []
    }
  }
  ...
  async getCableBoxes(): Promise<CableBoxDevice[]> {
    try {
      ...
      console.error('Error fetching cable boxes:', error.message)
      return []
    }
  }
  ...
  private async logCommand(
    ...
    try {
      ...
      console.error('Error logging CEC command:', error.message)
    }
  }
  ...
  async tuneChannel(cableBoxId: string, channel: string): Promise<CECCommandResult> {
    ...
    return this.queueCommand(box.devicePath, async () => {
      ...
      console.log(`[CEC] Tuning ${box.name} to channel ${channel} (${commands.length} commands)`)
      ...
      for (let i = 0; i < commands.length; i++) {
        ...
        if (!lastResult.success) {
          console.error(`[CEC] Failed to send command ${i + 1}/${commands.length}:`, lastResult.error)
          break
        }
