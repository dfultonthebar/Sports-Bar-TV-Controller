...
async function extractPDFContent(filePath: string): Promise<string> {
  try {
    console.log(`[TV Docs] Extracting content from PDF: ${filePath}`)
    ...
    console.log(`[TV Docs] Extracted ${data.numpages} pages, ${data.text.length} characters`)
    
    return data.text
  } catch (error: any) {
    console.error('[TV Docs] Error extracting PDF content:', error)
...
async function extractTextContent(filePath: string): Promise<string> {
  try {
    console.log(`[TV Docs] Reading text file: ${filePath}`)
    
    const content = await fs.readFile(filePath, 'utf-8')
    
    console.log(`[TV Docs] Read ${content.length} characters`)
    
    return content
  } catch (error: any) {
    console.error('[TV Docs] Error reading text file:', error)
...
export async function extractManualContent(filePath: string): Promise<string> {
  try {
    ...
    console.error('[TV Docs] Error extracting manual content:', error)
    throw error
  }
}
...
export function splitContentIntoChunks(content: string, chunkSize: number = 2000): string[] {
  ...
  console.log(`[TV Docs] Split content into ${chunks.length} chunks`)
...
export function extractKeySections(content: string): Record<string, string> {
  ...
  console.log(`[TV Docs] Extracted ${Object.keys(sections).length} sections`)
