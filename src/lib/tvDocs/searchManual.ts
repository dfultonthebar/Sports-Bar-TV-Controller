...
export async function searchTVManual(
  ...
  try {
    console.log(`[TV Docs] Searching for manual: ${manufacturer} ${model}`)
    ...
    for (const query of queries) {
      try {
        ...
        if (!searchResponse.ok) {
          console.warn(`[TV Docs] Search failed for query: ${query}`)
          continue
        }
        ...
        console.error(`[TV Docs] Error searching query "${query}":`, error)
      }
    }
    ...
    console.log(`[TV Docs] Found ${uniqueResults.length} potential manual sources`)
    
    return uniqueResults.slice(0, 10) // Return top 10 results
  } catch (error: any) {
    console.error('[TV Docs] Error searching for manual:', error)
...
export async function validateManualUrl(url: string): Promise<boolean> {
  try {
    ...
    console.error('[TV Docs] Error validating URL:', error)
    return false
  }
}
