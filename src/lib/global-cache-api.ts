...
class GlobalCacheAPI {
  ...
  async login(): Promise<boolean> {
    try {
      if (!this.credentials?.email || !this.credentials?.password) {
        console.warn('Global Cache credentials not provided, using demo mode')
        return false
      }
      ...
      console.error('Global Cache login error:', error)
      return false
    }
  }
  ...
  async getModelCodes(modelId: string): Promise<GlobalCacheCodeset[]> {
    try {
      ...
      console.error('Error getting model codes:', error)
      return this.getStandardCableBoxCodes()
    }
  }
