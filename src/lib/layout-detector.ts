...
export async function detectTVZonesFromImage(
  ...
  try {
    ...
    console.log(`[Layout Detector] Image loaded: ${imageWidth}x${imageHeight}, channels: ${info.channels}`)

    // Detect red regions (TV rectangles)
    const rectangles = await detectRedRectangles(data, imageWidth, imageHeight, info.channels)

    console.log(`[Layout Detector] Found ${rectangles.length} potential TV rectangles`)
    ...
    console.error('[Layout Detector] Error:', error)
    ...
    for (let x = 0; x < width; x += 3) {
      ...
      if (isRed(pixel.r, pixel.g, pixel.b)) {
        ...
        if (rect) {
          ...
          console.log(`[Detector] Found rectangle at (${rect.x}, ${rect.y}) size ${rect.width}x${rect.height}`)
        }
      }
    }
  }

  console.log(`[Detector] Total rectangles found before merging: ${rectangles.length}`)

  // Merge nearby rectangles that might be the same TV
  const mergedRectangles = mergeNearbyRectangles(rectangles, width, height)

  console.log(`[Detector] Total rectangles after merging: ${mergedRectangles.length}`)
  ...
  for (let i = 0; i < result.length; i++) {
    for (let j = i + 1; j < result.length; j++) {
      ...
      if (xOverlap > 0 && yOverlap > 0) {
        console.log(`[Zone Resolver] Zones ${i} and ${j} overlap by ${xOverlap.toFixed(2)}% x ${yOverlap.toFixed(2)}%`)
        ...
        if (dx > dy) {
          ...
          console.log(`[Zone Resolver] Adjusted horizontally: zone${i} width=${zone1.width.toFixed(2)}%, zone${j} width=${zone2.width.toFixed(2)}%`)
        } else {
          ...
          console.log(`[Zone Resolver] Adjusted vertically: zone${i} height=${zone1.height.toFixed(2)}%, zone${j} height=${zone2.height.toFixed(2)}%`)
        }
      }
    }
  }

  console.log(`[Zone Resolver] Resolved ${result.length} zones with minimum ${minSpacing}% spacing`)
      ...
      if (verticalDiff < 10) {
        ...
        if (zone1Right < zone2Right && zone1Left < zone2Left) {
          ...
          if (gap < MIN_HORIZONTAL_GAP) {
            console.log(`[Zone Adjuster] Zones ${zone1.label} and ${zone2.label} are too close (gap: ${gap.toFixed(2)}%)`)
            ...
            console.log(`[Zone Adjuster] Adjusted ${zone1.label} to x=${zone1.x.toFixed(2)}%, ${zone2.label} to x=${zone2.x.toFixed(2)}%`)
            isAdjacent = true
          }
        }
        // Check if zone1 is to the right of zone2
          ...
          if (gap < MIN_HORIZONTAL_GAP) {
            console.log(`[Zone Adjuster] Zones ${zone2.label} and ${zone1.label} are too close (gap: ${gap.toFixed(2)}%)`)
            ...
            console.log(`[Zone Adjuster] Adjusted ${zone2.label} to x=${zone2.x.toFixed(2)}%, ${zone1.label} to x=${zone1.x.toFixed(2)}%`)
            isAdjacent = true
          }
        }
      }
...
async function extractZonesFromRectangles(
  ...
  console.log(`[Zone Extractor] Expanded zones for touch-friendly interaction`)
