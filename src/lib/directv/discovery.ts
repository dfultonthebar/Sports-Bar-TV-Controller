...
export class DirecTVDiscovery {
  ...
  async identifyBox(ipAddress: string): Promise<DirecTVBox | null> {
    ...
    try {
      ...
      console.error(`Error identifying box at ${ipAddress}:`, error.message);
      return null;
    }
  }
  ...
  async discover(method: 'ssdp' | 'port_scan' | 'both' = 'both'): Promise<DiscoveryResult> {
    ...
    try {
      if (method === 'ssdp' || method === 'both') {
        ...
        console.log(`SSDP found ${ssdpIPs.length} potential devices`);
      }
      ...
      if (method === 'port_scan' || method === 'both') {
        ...
        console.log(`Port scan found ${scanIPs.length} devices`);
      }

      // Remove duplicates
      foundIPs = Array.from(new Set(foundIPs));
      console.log(`Total unique IPs found: ${foundIPs.length}`);

      // Identify each box
      const boxes: DirecTVBox[] = [];
      for (const ip of foundIPs) {
        try {
          console.log(`Identifying box at ${ip}...`);
          const box = await this.identifyBox(ip);
          if (box) {
            boxes.push(box);
            console.log(`✓ Identified ${box.model || 'Unknown'} at ${ip}`);
          }
        } catch (error: any) {
          errors.push(`Failed to identify ${ip}: ${error.message}`);
          console.error(`✗ Failed to identify ${ip}:`, error.message);
        }
      }
