  ...
  async log(entry: Omit<DirecTVLogEntry, 'timestamp'>): Promise<void> {
    try {
      ...
      if (fullEntry.ipAddress) {
        console.log(`${consolePrefix} [${fullEntry.ipAddress}:${fullEntry.port || 8080}] ${fullEntry.message}`)
      } else {
        console.log(`${consolePrefix} ${fullEntry.message}`)
      }

      if (fullEntry.details) {
        console.log('Details:', JSON.stringify(fullEntry.details, null, 2))
      }

      if (fullEntry.error) {
        console.error('Error:', fullEntry.error.message)
        if (fullEntry.error.stack) {
          console.error('Stack:', fullEntry.error.stack)
        }
      }

    } catch (error) {
      console.error('Failed to write to DirecTV log:', error)
    }
  }
