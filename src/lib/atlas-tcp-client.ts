...
export class AtlasTCPClient {
  ...
  async connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.socket.connect(this.port, this.host, () => {
        this.connected = true
        logger.info(`Connected to Atlas processor at ${this.host}:${this.port}`)
        resolve()
      })
      ...
      this.socket.on('close', () => {
        this.connected = false
        logger.info('Atlas TCP connection closed')
      })
    })
  }
  ...
  async setZoneSource(zoneIndex: number, sourceIndex: number): Promise<void> {
    ...
    logger.info(`Set Zone ${zoneIndex + 1} source to ${sourceIndex}`)
  }
  ...
  async setZoneVolume(zoneIndex: number, volume: number, usePercent: boolean = true): Promise<void> {
    ...
    await this.sendCommand(command)
    logger.info(`Set Zone ${zoneIndex + 1} volume to ${volume}${usePercent ? '%' : 'dB'}`)
  }
  ...
  async setZoneMute(zoneIndex: number, muted: boolean): Promise<void> {
    ...
    await this.sendCommand(command)
    logger.info(`${muted ? 'Muted' : 'Unmuted'} Zone ${zoneIndex + 1}`)
  }
  ...
  async subscribeToZone(zoneIndex: number): Promise<void> {
    ...
    await this.sendCommand(command)
    logger.info(`Subscribed to Zone ${zoneIndex + 1} parameters`)
  }
