...
export class EnhancedLogger {
  ...
  private async initializeLogger() {
    try {
      ...
      logger.error('Failed to initialize enhanced logger:', error)
    }
  }
  ...
  async log(entry: Omit<EnhancedLogEntry, 'id' | 'timestamp'>): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to write log entry:', error)
    }
  }
  ...
  async getRecentLogs(hours: number = 24, category?: LogCategory, level?: LogLevel): Promise<EnhancedLogEntry[]> {
    try {
      ...
      logger.error(`Failed to read logs from ${category || 'all'}:`, error)
      return []
    }
  }
