...
export async function getApiKeysByNames(provider: string, names: string[]): Promise<Record<string, string>> {
  try {
    ...
    logger.error(`Error fetching API keys by names for provider ${provider}:`, error)
    return {}
  }
}
...
export async function getGracenoteConfig(): Promise<{
  ...
  try {
    ...
    logger.error('Error getting Gracenote config:', error)
    return {}
  }
}
...
export async function getSpectrumBusinessConfig(): Promise<{
  ...
  try {
    ...
    logger.error('Error getting Spectrum Business config:', error)
    return {}
  }
}
...
export async function isProviderConfigured(provider: string): Promise<boolean> {
  try {
    ...
    logger.error(`Error checking if provider ${provider} is configured:`, error)
    return false
  }
}
