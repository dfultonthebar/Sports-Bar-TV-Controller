...
class ConfigChangeTracker {
  ...
  async initializeWatching() {
    ...
    for (const configFile of configFiles) {
      ...
      try {
        ...
        logger.info(`Config file doesn't exist yet: ${configFile}`)
      }
    }
  }
  ...
  private watchFile(fullPath: string, configFile: string) {
    try {
      ...
      logger.error(`Error watching file ${configFile}:`, error)
    }
  }
  ...
  private async handleFileChange(fullPath: string, configFile: string) {
    try {
      ...
      logger.error(`Error handling file change for ${configFile}:`, error)
    }
  }
  ...
  private async checkAutoSync(changeEvent: ConfigChangeEvent) {
    try {
      ...
      logger.error('Error in auto-sync check:', error)
    }
  }
  ...
  cleanup() {
    for (const [file, watcher] of Array.from(this.watchers.entries())) {
      try {
        ...
        logger.error(`Error closing watcher for ${file}:`, error)
      }
    }
    this.watchers.clear()
  }
