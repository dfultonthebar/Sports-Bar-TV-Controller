...
async function fetchEnhancedStreamingData(platformId: string, credential: StreamingCredential): Promise<any[]> {
  try {
    logger.info(`🔍 Fetching enhanced data from ${platformId}`)
    ...
    logger.error(`Error fetching enhanced data from ${platformId}:`, error)
    return []
  }
}
...
export async function getEnhancedStreamingSportsData(selectedLeagues: string[]): Promise<{
  ...
  logger.info('🎯 Getting enhanced streaming sports data...')
  
  const credentials = loadStreamingCredentials()
  const activeCredentials = credentials.filter(c => c.status === 'active')
  
  logger.info(`📊 Found ${activeCredentials.length} active streaming credentials`)
  ...
  for (const credential of activeCredentials) {
    try {
      logger.info(`🔄 Processing platform: ${credential.platformId}`)
      ...
      if (platformGames.length > 0) {
        ...
        logger.info(`✅ Added ${platformGames.length} enhanced games from ${credential.platformId}`)
      }
    } catch (error) {
      logger.error(`❌ Error processing platform ${credential.platformId}:`, error)
    }
  }
  ...
  logger.info(`🎉 Retrieved ${allEnhancedGames.length} enhanced games from ${enhancedPlatforms.length} platforms`)
