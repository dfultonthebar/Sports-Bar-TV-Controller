...
class EnhancedSpectrumChannelService {
    ...
    try {
      ...
      if (this.apiConfigured) {
        try {
          ...
          logger.warn('Spectrum API failed, using enhanced fallback:', apiError)
      ...
      logger.error('Error in enhanced channel lineup:', error)
      return {
        success: false,
        channels: [] as any[],
        lastUpdated: new Date().toISOString(),
  ...
  async updateChannelLineup(): Promise<boolean> {
    try {
      ...
      this.lastUpdated = new Date()
      logger.info('✅ Enhanced channel lineup updated successfully')
      return true
    } catch (error) {
      logger.error('❌ Error updating enhanced channel lineup:', error)
      return false
    }
  }
