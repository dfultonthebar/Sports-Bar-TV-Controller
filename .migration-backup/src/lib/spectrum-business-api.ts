...
class SpectrumBusinessApiService {
  ...
  async getAccountInfo(): Promise<any> {
    ...
    if (!(await this.isConfigured())) {
      logger.warn('Spectrum Business API not configured')
    ...
    try {
      ...
      logger.error('Spectrum Business account error:', error)
      return this.getFallbackAccountInfo()
    }
  }
  ...
  async getChannelLineup(): Promise<SpectrumBusinessChannel[]> {
    ...
    if (!(await this.isConfigured())) {
      logger.warn('Spectrum Business API not configured - using enhanced fallback')
    ...
    try {
      ...
      logger.error('Spectrum Business channel lineup error:', error)
      return this.getEnhancedFallbackChannels()
    }
  }
  ...
  async getGuideData(
    ...
    try {
      ...
      logger.error('Spectrum Business guide data error:', error)
      return this.getFallbackGuideData(startTime, endTime)
    }
  }
