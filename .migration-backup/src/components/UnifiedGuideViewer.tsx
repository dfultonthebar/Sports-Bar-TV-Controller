...
export default function UnifiedGuideViewer({ 
  ...
  const loadCachedData = async () => {
    try {
      ...
      logger.warn('No cached guide data available')
    }
  }
  ...
  const fetchGuideData = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        logger.info(`✅ Fetched guide data: ${result.data.summary.totalPrograms} programs from ${result.data.summary.successfulFetches} devices`)
      } else {
