...
export default function IRDeviceControl() {
  ...
  const checkITachConnection = async () => {
    try {
      ...
      logger.error('Failed to check iTach connection:', error)
    }
  }
  ...
  const searchCodesets = async (brand: string, deviceType: string) => {
    try {
      ...
      if (response.ok) {
        ...
        logger.info(`Found ${data.codesets?.length || 0} codesets for ${brand} ${deviceType}`)
        if (data.globalCacheApiUsed) {
          logger.info('Using Global Cache IR Database API')
        }
      }
    } catch (error) {
      logger.error('Failed to search codesets:', error)
    }
  }
  ...
  const searchAllSpectrumModels = async () => {
    try {
      ...
      if (response.ok) {
        ...
        logger.info(`Found ${data.totalModels} Spectrum cable box models across all manufacturers`)
      }
    } catch (error) {
      logger.error('Failed to search all Spectrum models:', error)
    }
  }
  ...
  const sendIRCommand = async (command: string) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        logger.info('Command sent:', data.message)
      } else {
        const error = await response.json()
  ...
  const toggleLearningMode = async () => {
    try {
      ...
      logger.error('Failed to toggle learning mode:', error)
    }
  }
