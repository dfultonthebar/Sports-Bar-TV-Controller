...
async function ensureManualsDirectory(): Promise<void> {
  try {
    ...
    logger.info(`[TV Docs] Created manuals directory: ${MANUALS_DIR}`)
  }
}
...
async function downloadPDF(url: string, outputPath: string): Promise<void> {
  try {
    logger.info(`[TV Docs] Downloading PDF from: ${url}`)
    ...
    logger.info(`[TV Docs] PDF saved to: ${outputPath}`)
  } catch (error: any) {
    logger.error('[TV Docs] Error downloading PDF:', error)
...
async function downloadHTML(url: string, outputPath: string): Promise<void> {
  try {
    logger.info(`[TV Docs] Fetching HTML from: ${url}`)
    ...
    logger.info(`[TV Docs] HTML content saved to: ${outputPath}`)
  } catch (error: any) {
    logger.error('[TV Docs] Error downloading HTML:', error)
    ...
    for (const result of searchResults) {
      try {
        ...
        try {
          ...
          logger.info(`[TV Docs] Manual already exists: ${outputPath}`)
        ...
        if (stats.size > 1000) { // At least 1KB
          logger.info(`[TV Docs] Successfully downloaded manual (${stats.size} bytes)`)
          return { path: outputPath, url: result.url }
        } else {
          logger.warn(`[TV Docs] Downloaded file too small, trying next source`)
          await fs.unlink(outputPath).catch(() => {})
        }
      } catch (error) {
        logger.error(`[TV Docs] Failed to download from ${result.url}:`, error)
        // Continue to next result
      }
    }

    logger.warn(`[TV Docs] Could not download manual from any source`)
    return null
  } catch (error: any) {
    logger.error('[TV Docs] Error in downloadTVManual:', error)
...
export async function getManualPath(manufacturer: string, model: string): Promise<string | null> {
  try {
    ...
    logger.error('[TV Docs] Error checking manual path:', error)
    return null
  }
}
...
export async function listDownloadedManuals(): Promise<Array<{ filename: string; size: number; path: string }>> {
  try {
    ...
    logger.error('[TV Docs] Error listing manuals:', error)
    return []
  }
}
