...
export class ESPNApiClient {
  ...
  constructor(config: ESPNApiConfig = {}) {
    ...
    logger.info('[ESPN API] Client initialized')
  }
  ...
  public async getScoreboard(sport: string, league: string, options?: {
    ...
    try {
      logger.info(`[ESPN API] Getting scoreboard for ${sport}/${league}`)
      ...
      logger.error(`[ESPN API] Error getting scoreboard:`, error)
      throw error
    }
  }
  ...
  public async getTeam(sport: string, league: string, teamId: string): Promise<any> {
    try {
      logger.info(`[ESPN API] Getting team ${teamId} for ${sport}/${league}`)
      ...
      logger.error(`[ESPN API] Error getting team:`, error)
      throw error
    }
  }
  ...
  public async getEvent(sport: string, league: string, eventId: string): Promise<any> {
    try {
      logger.info(`[ESPN API] Getting event ${eventId} for ${sport}/${league}`)
      ...
      logger.error(`[ESPN API] Error getting event:`, error)
      throw error
    }
  }
  ...
  public async getLiveEvents(): Promise<ESPNEvent[]> {
    try {
      logger.info('[ESPN API] Getting live events')
      ...
      for (const { sport, league } of leagues) {
        try {
          ...
          logger.error(`[ESPN API] Error checking ${sport}/${league} for live events:`, error)
        }
      }

      logger.info(`[ESPN API] Found ${liveEvents.length} live events`)
      return liveEvents
    } catch (error) {
      logger.error('[ESPN API] Error getting live events:', error)
      return []
    }
  }
  ...
  public async searchEventsByTeam(teamName: string, sport?: string): Promise<ESPNEvent[]> {
    try {
      logger.info(`[ESPN API] Searching for events with team: ${teamName}`)
      ...
      for (const { sport, league } of leaguesToCheck) {
        try {
          ...
          logger.error(`[ESPN API] Error searching ${sport}/${league}:`, error)
        }
      }

      logger.info(`[ESPN API] Found ${matchingEvents.length} matching events`)
      return matchingEvents
    } catch (error) {
      logger.error('[ESPN API] Error searching events:', error)
      return []
    }
  }
