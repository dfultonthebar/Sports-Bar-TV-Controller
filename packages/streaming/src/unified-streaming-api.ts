...
/**
 * Installed streaming app information
 */
...
/**
 * FireTV adapter interface for dependency injection
 */
export interface FireTVAdapter {
  getInstalledApps(deviceId: string, ipAddress: string, port?: number): Promise<InstalledStreamingApp[]>
}
...
class UnifiedStreamingAPI {
  ...
  private firetvAdapter?: FireTVAdapter

  private constructor(firetvAdapter?: FireTVAdapter) {
    this.firetvAdapter = firetvAdapter
    logger.info('[UNIFIED API] Initializing Unified Streaming API')
  }
  ...
  public async getAllLiveEvents(): Promise<UnifiedEvent[]> {
    logger.info('[UNIFIED API] Getting all live events')
    ...
    try {
      ...
      logger.error('[UNIFIED API] Error getting ESPN live events:', error)
    }
    ...
    try {
      ...
      logger.error('[UNIFIED API] Error getting MLB live events:', error)
    }
    ...
    try {
      ...
      logger.error('[UNIFIED API] Error getting NFHS live events:', error)
    }

    logger.info(`[UNIFIED API] Found ${events.length} total live events`)
    return events
  }
  ...
  public async getUpcomingEvents(sport?: string, days: number = 7): Promise<UnifiedEvent[]> {
    logger.info(`[UNIFIED API] Getting upcoming events${sport ? ` for ${sport}` : ''}`)
    ...
    try {
      ...
      logger.error('[UNIFIED API] Error getting ESPN events:', error)
    }
    ...
    try {
      ...
      logger.error('[UNIFIED API] Error getting MLB events:', error)
    }
    ...
    try {
      ...
      logger.error('[UNIFIED API] Error getting NFHS events:', error)
    }

    logger.info(`[UNIFIED API] Found ${events.length} upcoming events`)
    return events
  }
  ...
  public async searchEventsByTeam(teamName: string, sport?: string): Promise<UnifiedEvent[]> {
    logger.info(`[UNIFIED API] Searching for events with team: ${teamName}`)
    ...
    try {
      ...
      logger.error('[UNIFIED API] Error searching ESPN:', error)
    }
    ...
    try {
      ...
      logger.error('[UNIFIED API] Error searching MLB:', error)
    }
    ...
    try {
      ...
      logger.error('[UNIFIED API] Error searching NFHS:', error)
    }

    logger.info(`[UNIFIED API] Found ${events.length} events for ${teamName}`)
    return events
  }
  ...
  public async getEventsForInstalledApps(
    ...
    logger.info(`[UNIFIED API] Getting events for installed apps on device ${deviceId}`)

    // Check if adapter is available
    if (!this.firetvAdapter) {
      logger.warn('[UNIFIED API] FireTV adapter not configured, cannot get installed apps')
      return []
    }

    // Get installed streaming apps
    const installedApps = await this.firetvAdapter.getInstalledApps(deviceId, ipAddress, port)
    ...
    logger.info(`[UNIFIED API] Found events for ${results.length} installed apps`)
    return results
  }
    ...
    switch (appId) {
      ...
      case 'mlb-tv':
        try {
          ...
          logger.error('[UNIFIED API] Error getting MLB.TV events:', error)
        }
        break
...
/**
 * Export singleton instance (without adapter - adapter can be provided via getInstance)
 */
