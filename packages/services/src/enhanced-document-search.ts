 ...
 * Provides fuzzy search with keyword extraction for document content
 */

import { db, schema, isNotNull, or, eq } from '@sports-bar/database'
import { logger } from '@sports-bar/logger'
import { extractTextFromFile } from '@sports-bar/utils'
...
export class EnhancedDocumentSearch {
  ...
  async searchDocuments(query: string, limit: number = 5): Promise<DocumentSearchResult[]> {
    try {
      ...
      logger.error('Enhanced document search error:', { error })
      return []
    }
  }
    ...
    try {
      ...
      for (const doc of documents) {
        try {
          ...
          logger.error(`Failed to reprocess document ${doc.originalName}:`, { error })
          errors++
        }
      }
      ...
      logger.error('Failed to reprocess documents:', { error })
      throw error
    }
  }
}
