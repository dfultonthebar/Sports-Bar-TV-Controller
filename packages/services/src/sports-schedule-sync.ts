import { findMany, findFirst, update, eq, or, lt, and, db, schema } from '@sports-bar/database'
    ...
    for (let i = 0; i < homeTeams.length; i += batchSize) {
      ...
      for (let j = 0; j < results.length; j++) {
        ...
        if (result.status === 'fulfilled') {
          ...
          logger.error(`[Sports Sync] Error syncing ${team.teamName}`, { error: result.reason })
          logs.push({
            team: team.teamName,
            league: team.league,
            success: false,
