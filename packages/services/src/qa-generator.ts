  ...
  try {
    ...
    const processFile = async (file: string, index: number) => {
      try {
        ...
        if (!shouldProcess) {
          logger.debug(`[${index + 1}/${files.length}] Skipping (${reason}): ${file}`);
          skippedFiles++;
    ...
    logger.debug(`Summary: ${processedFiles} processed, ${skippedFiles} skipped, ${failedFiles} failed`);
    ...
    logger.debug(`Completed in ${elapsedTime}s`);
...
function parseGeneratedQAs(response: string, sourceFile: string): GeneratedQA[] {
  try {
    ...
    try {
      ...
      logger.error(`JSON parse error for ${sourceFile}`, { error: parseError });
      return [];
    }
