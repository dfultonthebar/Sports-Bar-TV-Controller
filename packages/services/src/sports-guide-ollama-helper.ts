...
export async function getSportsGuideLogs(lines: number = 100): Promise<string> {
  try {
    ...
    logger.error('[Ollama-Helper] Error fetching sports guide logs:', { error })
    return ''
  }
}
...
export async function getSportsGuideErrorLogs(lines: number = 50): Promise<string> {
  try {
    ...
    logger.error('[Ollama-Helper] Error fetching sports guide error logs:', { error })
    return ''
  }
}
...
export async function queryOllamaWithContext(
  ...
  try {
    logger.info(`[Ollama-Helper] Querying Ollama with: "${query}"`)
...
`
    ...
    logger.info(`[Ollama-Helper] Ollama response received (${data.response?.length || 0} chars)`)
    ...
    logger.error('[Ollama-Helper] Error querying Ollama:', { error })
    return {
      success: false,
...
export async function analyzeSportsGuideLogs(): Promise<{
  ...
  try {
    logger.info('[Ollama-Helper] Analyzing sports guide logs with Ollama...')
...
Be concise and focus on actionable insights.`
    ...
    logger.info('[Ollama-Helper] Log analysis completed successfully')
    ...
    logger.error('[Ollama-Helper] Error analyzing logs:', { error })
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error'
    }
...
export async function getSportsGuideRecommendations(
  ...
  try {
    logger.info('[Ollama-Helper] Getting sports guide recommendations...')
...
Be concise and helpful.`
    ...
    logger.info('[Ollama-Helper] Recommendations generated successfully')
    ...
    logger.error('[Ollama-Helper] Error getting recommendations:', { error })
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error'
    }
...
export async function testOllamaConnection(): Promise<{
  ...
  try {
    logger.info('[Ollama-Helper] Testing Ollama connection...')
    ...
    logger.info(`[Ollama-Helper] Ollama connection successful (${responseTime}ms)`)
    ...
    logger.error('[Ollama-Helper] Ollama connection failed:', { error })
    return {
      success: false,
