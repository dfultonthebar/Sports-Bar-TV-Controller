...
import { create, eq, findUnique, update, upsert, schema } from '@sports-bar/database';
  ...
  try {
    ...
    const processFile = async (file: string, index: number) => {
      try {
        ...
        if (!shouldProcess) {
          logger.info(`[${index + 1}/${files.length}] Skipping (${reason}): ${file}`);
          skippedFiles++;
        ...
        if (fileSizeMB > MAX_FILE_SIZE_MB) {
          logger.warn(`[${index + 1}/${files.length}] Skipping large file (${fileSizeMB.toFixed(2)}MB): ${file}`);
        ...
        for (const qa of qas) {
          try {
            ...
            logger.error(`Error saving Q&A to database`, { error: dbError });
    ...
    logger.info(`Summary: ${processedFiles} processed, ${skippedFiles} skipped, ${failedFiles} failed, ${generatedQAs} Q&As generated`);
    ...
    logger.info(`Completed in ${elapsedTime}s`);
