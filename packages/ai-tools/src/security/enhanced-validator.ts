...
export class EnhancedSecurityValidator {
  ...
  private readonly DANGEROUS_PATTERNS: Array<{ pattern: RegExp; severity: string; description: string }> = [
  ...
  validateCodeExecution(request: CodeExecutionRequest): SecurityValidation {
    ...
    for (const dangerousPattern of this.DANGEROUS_PATTERNS) {
      if (dangerousPattern.pattern.test(code)) {
        return {
          allowed: false,
          reason: `Code contains dangerous pattern: ${dangerousPattern.description}`,
        };
      }
    }
