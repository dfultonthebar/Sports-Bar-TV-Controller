/**
 * Centralized Hardware Configuration (Package-level)
 *
 * Mirror of apps/web/src/lib/hardware-config.ts for use by shared packages
 * that cannot import from @/lib. Both files define the same constants so
 * any hardware change only requires updating these two files.
 *
 * Usage:
...
export const HARDWARE_CONFIG = {
  ...
  wolfpack: {
    audioOutputStart: 37,
    audioOutputCount: 4,
    audioOutputSlots: [37, 38, 39, 40] as readonly number[],
  },
  api: {
