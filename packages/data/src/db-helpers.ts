 ...
 */
export interface DbHelperDependencies {
  db: any
  schema: any
  logger: Logger
}
...
/**
 * Create database helpers with injected dependencies
 */
export function createDbHelpers(deps: DbHelperDependencies) {
  const { db, schema, logger } = deps
   ...
   */
  async function findMany<T extends TableName>(
    tableName: T,
    options?: {
      where?: any
      orderBy?: any | any[]
    ...
    const displayName = getTableDisplayName(tableName as string)
   ...
   */
  async function findFirst<T extends TableName>(
    tableName: T,
    options?: {
      where?: any
      orderBy?: any | any[]
   ...
   */
  async function findUnique<T extends TableName>(
    tableName: T,
    where: any
  ) {
    const table = schema[tableName] as any
   ...
   */
  async function create<T extends TableName>(
    tableName: T,
    data: any
  ) {
    const table = schema[tableName] as any
   ...
   */
  async function createMany<T extends TableName>(
    tableName: T,
    data: any[]
  ) {
    const table = schema[tableName] as any
   ...
   */
  async function updateMany<T extends TableName>(
    tableName: T,
    where: any,
    data: any
  ) {
   ...
   */
  async function deleteRecord<T extends TableName>(
    tableName: T,
    where: any
  ) {
    const table = schema[tableName] as any
   ...
   */
  async function deleteMany<T extends TableName>(
    tableName: T,
    where: any
  ) {
    const table = schema[tableName] as any
   ...
   */
  async function count<T extends TableName>(
    tableName: T,
    where?: any
  ) {
    const table = schema[tableName] as any
   ...
   */
  async function executeRaw(query: string, params?: any[]) {
  ...
  return {
    ...
    executeRaw,
    transaction,
    // Re-export db and schema for direct access
    db,
    schema
  }
}

// Export the type of the returned helpers
