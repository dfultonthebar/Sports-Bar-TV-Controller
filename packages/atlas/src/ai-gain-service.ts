 ...
 * AI Gain Service
 *
 * Provides AI-powered automatic gain adjustment for Atlas audio processors.
 * Monitors input levels and adjusts gain to maintain target levels.
 */
...
export class AIGainService {
  ...
  async startMonitoring(processorId: string): Promise<void> {
    logger.info(`Starting AI gain monitoring for processor ${processorId}`)
    ...
    const interval = setInterval(async () => {
      try {
        ...
        logger.error(`Error in AI gain monitoring for ${processorId}`, { error })
      }
    }, 500)

    this.monitoringIntervals.set(processorId, interval)
    logger.info(`AI gain monitoring started for processor ${processorId}`)
  }
  ...
  stopMonitoring(processorId: string): void {
    ...
    if (interval) {
      ...
      logger.info(`AI gain monitoring stopped for processor ${processorId}`)
    }
  ...
  private async processAIGainAdjustments(processorId: string): Promise<void> {
    ...
    for (const config of aiConfigs) {
      try {
        ...
        logger.error(`Error processing input ${config.inputNumber}`, { error })
      }
    }
  }
  ...
  private async processInputAdjustment(processor: any, config: any): Promise<void> {
    ...
    if (newGain === currentGain) {
      logger.info(`Input ${config.inputNumber}: Gain limit reached (${currentGain}dB)`)
      return
    }
  ...
  private async setInputGain(processor: any, inputNumber: number, gain: number): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      client.connect(5321, processor.ipAddress, () => {
        ...
        logger.info(`[AI Gain Service] Setting input ${inputNumber} (atlas index ${atlasIndex}) gain to ${gain}dB`)
