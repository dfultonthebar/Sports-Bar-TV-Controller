...
import { db, schema, eq, and, desc, findFirst, findMany, findUnique, create, update, upsert, not } from '@sports-bar/database'
  ...
  async sideloadApp(
    ...
    try {
      ...
      const sourceDevice = await findUnique('fireCubeDevices', {
        where: eq(schema.fireCubeDevices.id, sourceDeviceId)
      })
      ...
      const app = await findFirst('fireCubeApps', {
        where: and(
          eq(schema.fireCubeApps.deviceId, sourceDeviceId),
          eq(schema.fireCubeApps.packageName, packageName)
        )
      })
  ...
  private async performSideload(
    ...
    try {
      ...
      for (let i = 0; i < targetDeviceIds.length; i++) {
        ...
        try {
          const targetDevice = await findUnique('fireCubeDevices', {
            where: eq(schema.fireCubeDevices.id, targetDeviceId)
          })
          ...
          if (installSuccess) {
            ...
            await upsert('fireCubeApps',
              and(
                eq(schema.fireCubeApps.deviceId, targetDeviceId),
                eq(schema.fireCubeApps.packageName, packageName)
              ),
              {
      ...
      await update('fireCubeSideloadOperations',
        eq(schema.fireCubeSideloadOperations.id, operationId),
        {
          status: finalStatus,
          progress: 100,
          completedDevices,
  ...
  async getOperationStatus(operationId: string): Promise<any> {
    try {
      const operation = await findUnique('fireCubeSideloadOperations', {
        where: eq(schema.fireCubeSideloadOperations.id, operationId)
      })
  ...
  async getAllOperations(limit: number = 50): Promise<any[]> {
    try {
      const operations = await findMany('fireCubeSideloadOperations', {
        orderBy: [desc(schema.fireCubeSideloadOperations.startedAt)],
        limit
      })
  ...
  async cloneDevice(sourceDeviceId: string, targetDeviceId: string): Promise<string> {
    try {
      ...
      const apps = await findMany('fireCubeApps', {
        where: and(
          eq(schema.fireCubeApps.deviceId, sourceDeviceId),
          eq(schema.fireCubeApps.isSystemApp, false)
        )
      })
  ...
  async syncAppToAllDevices(sourceDeviceId: string, packageName: string): Promise<string> {
    try {
      ...
      const devices = await findMany('fireCubeDevices', {
        where: and(
          not(eq(schema.fireCubeDevices.id, sourceDeviceId)),
          eq(schema.fireCubeDevices.status, 'online')
        )
      })
