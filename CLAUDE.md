...
**AI Suggest Fire TV grounding + collision repair (v2.84.0):** Every LLM pick is gated by `canAirGame(input, game)` in `ai-suggest/route.ts:parseOllamaResponse` — cable needs a resolved cable channel, directv a directv channel, **firetv needs the game's streaming app INSTALLED on that exact box.** The per-box installed-app set is `input_sources.available_networks` UNIONed with the operator-refreshed `DeviceSubscription.subscriptions` JSON (keyed by `deviceId`), fuzzy-matched so catalog app labels ("Prime Video") match installed entries ("Amazon Prime Video"). A cable/sat game (no `streamingApp`) is therefore NEVER airable on a Fire TV. When a pick can't air its game (the old `no_route`) OR collides with another in-batch pick (the old `in_batch_collision`), `pickCompatibleInput()` GREEDILY reassigns it to an idle compatible input (cable/directv box, or another Fire TV with the app) instead of hard-rejecting — only rejects when nothing idle can air it. Fewer-but-routable beats an all-rejected empty slate. Catalog Fire TV games stay on their locked input (Scout-proven, keeps the deepLink); a cross-box spread DROPS the deepLink (it's device-specific → app launches fresh). If a box's `DeviceSubscription` row is empty, its Fire TVs can only ride their catalog-locked input (safe, just no cross-box repair) — keep those rows fresh.

**Schedule-end revert to defaults (window = 15 min, v2.84.1):** When `auto-reallocator` ends an allocation (game over), it reverts the box to defaults IF nothing else is scheduled for that input within the next **15 minutes** (`revertSkipHorizon` in `auto-reallocator.ts:revertTVsToDefaults`; was 30 min). Revert = (a) every TV output routed back to its default input via `/api/matrix/route` (outputDefaults → roomDefaults → globalDefault), AND (b) the box tuned back to its default channel via `/api/channel-presets/tune`. Handles BOTH cable (IR, resolved via `IRDevice.matrixInput`) AND **DirecTV (IP, resolved via `DirecTVDevice.inputChannel`)** — both keyed into `cableBoxDefaults` by matrix-input number (Holmgren: keys 1-4 cable, 5-10 DirecTV). No configured default → auto-seeds from the most recent successful `ChannelTuneLog` before game start. A confirmed-live game (ESPN live-status) holds its TVs indefinitely and is never reverted (v2.82.52). The end-detection buffer (`estimated_end + 30min`, Check 3) is a SEPARATE concern from this revert lookahead — don't conflate them.
