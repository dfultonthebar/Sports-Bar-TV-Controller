...
8. **Read `docs/VERSION_SETUP_GUIDE.md` on every version-changing update.** Before trusting a version bump, scan the entry for the target version in that file. Each entry lists REQUIRED manual setup for that version — install Ollama, seed a DB table, set a new env var, etc. Checkpoint B of auto-update must verify each "Required manual step" was completed OR perform it automatically (if safe) OR flag it for the operator in the decision response. `LOCATION_UPDATE_NOTES.md` tells you WHETHER to update; `VERSION_SETUP_GUIDE.md` tells you WHAT TO DO so it works after updating. When adding a new version to main, you MUST add an entry to this guide in the same commit as the `package.json` bump — skipping it means future updates will silently miss required setup.
