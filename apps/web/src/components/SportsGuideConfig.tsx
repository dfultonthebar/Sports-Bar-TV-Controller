...
export default function SportsGuideConfig() {
  ...
  if (loading) {
    return (
      <div className="rounded-lg border border-slate-700 bg-slate-900/40 p-6">
        <h2 className="text-xl font-semibold text-white mb-4">Sports Guide API Configuration</h2>
  ...
  return (
    <div className="rounded-lg border border-slate-700 bg-slate-900/40 p-6">
      ...
      {/* API Status Section */}
      <div className="mb-6">
        <h3 className="text-lg font-medium text-white mb-3">API Status</h3>
          ...
          {status?.configured && (
            <>
              <div className="flex items-center justify-between">
                ...
                <span className="text-white font-mono text-sm">{status.apiUrl}</span>
              </div>

              <div className="flex items-center justify-between">
                <span className="text-slate-300">User ID:</span>
                <span className="text-white font-mono text-sm">{status.userId}</span>
              </div>
      ...
      {status?.configured && (
        <div className="mb-6">
          <h3 className="text-lg font-medium text-white mb-3">API Key Verification</h3>
          <button
            onClick={handleVerifyKey}
            disabled={verifying}
            className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 disabled:bg-slate-600 disabled:cursor-not-allowed transition-colors"
          >
            {verifying ? (
          ...
          {verificationResult && (
            <div className={`mt-3 p-4 rounded-lg ${
              verificationResult.valid
                ? 'bg-green-950/30 border border-slate-700'
                : 'bg-red-950/30 border border-slate-700'
            }`}>
      ...
      <div className="mb-6">
        <h3 className="text-lg font-medium text-white mb-3">Update API Configuration</h3>

        {!showUpdateForm ? (
          <button
            onClick={() => setShowUpdateForm(true)}
            className="border border-slate-600 text-slate-300 px-4 py-2 rounded-lg hover:bg-slate-700 transition-colors"
          >
            {status?.configured ? 'Change API Key' : 'Configure API Key'}
          </button>
        ) : (
          <form onSubmit={handleUpdateKey} className="bg-slate-800/50 rounded-lg p-4 space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="258351"
                className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                required
              />
            </div>
            ...
            {updateMessage && (
              <div className={`p-3 rounded-lg ${
                updateMessage.type === 'success'
                  ? 'bg-green-950/30 border border-slate-700 text-green-400'
                  : 'bg-red-950/30 border border-slate-700 text-red-400'
              }`}>
                {updateMessage.text}
              </div>
            )}
      ...
      {/* Information Section */}
      <div className="bg-blue-950/30 border border-slate-700 rounded-lg p-4">
        <h4 className="text-sm font-medium text-blue-400 mb-2">About Sports Guide API</h4>
