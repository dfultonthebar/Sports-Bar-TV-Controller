      ...
      <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-4">
        ...
        {liveNow.length > 0 ? (
          <div className="flex gap-3 overflow-x-auto pb-2 snap-x snap-mandatory scrollbar-thin scrollbar-thumb-slate-700">
            {liveNow.map((game, i) => (
              <div key={i} className="shrink-0 w-[280px] snap-start rounded-lg border border-slate-700 bg-slate-800/80 p-3">
                {/* League badge */}
                ...
                <div className="flex items-center gap-2 mt-2 pt-2 border-t border-slate-700">
                  {game.primaryNetwork && (
                    <span className="text-xs text-slate-400 bg-slate-700/50 px-2 py-0.5 rounded-sm">{game.primaryNetwork}</span>
                  )}
                  <div className="flex gap-1 ml-auto">
                    {game.cableChannel && (
                      <button
                        onClick={() => handleTuneChannel(game.cableChannel!, 'cable')}
                        className="text-xs px-2 py-1 rounded-sm bg-green-600/20 text-green-400 border border-green-500/30 hover:bg-green-600/30 transition-colors"
                      >
                    ...
                    {game.streamingApp && selectedDeviceType === 'streaming' && (
                      <button
                        ...
                        className="text-xs px-2 py-1 rounded-sm bg-blue-600/20 text-blue-400 border border-blue-500/30 hover:bg-blue-600/30 transition-colors"
                      >
          ...
          <div className="space-y-2">
            {comingUp.map((game, i) => (
              <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-slate-800/60 border border-slate-700/50">
                <span className="text-xs text-amber-400 font-mono w-16 shrink-0">{game.gameTime}</span>
                <span className={`text-xs font-medium px-1.5 py-0.5 rounded-sm border shrink-0 ${LEAGUE_COLORS[game.league] || 'bg-slate-500/20 text-slate-400 border-slate-500/30'}`}>
                  {game.league}
                </span>
                ...
                {game.primaryNetwork && (
                  <span className="text-xs text-slate-400 bg-slate-700/50 px-1.5 py-0.5 rounded-sm shrink-0">{game.primaryNetwork}</span>
                )}
                <div className="flex gap-1 shrink-0">
                  {game.cableChannel && (
                    <button
                      onClick={() => handleTuneChannel(game.cableChannel!, 'cable')}
                      className="text-xs px-1.5 py-0.5 rounded-sm bg-green-600/20 text-green-400 border border-green-500/30 hover:bg-green-600/30"
                    >
          ...
          {showFullSchedule && (
            <div className="mt-3 space-y-1.5">
              {todaySchedule.map((game, i) => (
                <div key={i} className="flex items-center gap-3 px-3 py-1.5 rounded-sm bg-slate-800/40 text-sm">
                  <span className="text-xs text-slate-500 font-mono w-16 shrink-0">{game.gameTime}</span>
                  <span className={`text-xs font-medium px-1.5 py-0.5 rounded-sm border shrink-0 ${LEAGUE_COLORS[game.league] || 'bg-slate-500/20 text-slate-400 border-slate-500/30'}`}>
                    {game.league}
                  </span>
                  <span className="text-slate-300 truncate flex-1">{game.awayAbbrev} @ {game.homeAbbrev}</span>
                  {game.primaryNetwork && (
                    <span className="text-xs text-slate-500 shrink-0">{game.primaryNetwork}</span>
                  )}
                </div>
              ))}
            </div>
