...
export default function BartenderRemoteControl() {
  ...
  useEffect(() => {
    ...
    if (selectedLeagues.length > 0 && showSportsGuide) {
      logger.info('🏆 Auto-loading sports guide due to state change')
      generateSportsGuide()
    }
  ...
  const loadAudioProcessors = async () => {
    try {
      ...
      logger.error('Error loading audio processors:', error)
    }
  }
  ...
  const controlAudioZone = async (action: string, zone: AudioZone, value?: any) => {
    ...
    try {
      ...
      logger.error('Error controlling audio zone:', error)
  ...
  const generateSportsGuide = async () => {
    ...
    try {
      ...
      if (result.success && result.data && result.data.games) {
        ...
        logger.error('Failed to generate sports guide:', result.error || 'No games data')
        setSportsGuide([])
        setSportsGuideStatus(`Failed to load games: ${result.error || 'No data'}`)
      }
    } catch (error) {
      logger.error('Error generating sports guide:', error)
      setSportsGuide([])
