...
export default function ConflictAlerts({
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      ...
      default:
        return 'border-slate-600 bg-slate-900/30';
    }
  };
  ...
  if (loading) {
    return (
      <div className="rounded-lg border border-slate-700 bg-slate-900/40 p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Scheduling Conflicts</h3>
        <p className="text-sm text-slate-400">Checking for conflicts...</p>
      </div>
    );
  }
