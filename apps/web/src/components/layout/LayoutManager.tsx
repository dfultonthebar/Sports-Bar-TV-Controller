...
export default function LayoutManager({
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
      {/* Header */}
      <div className="bg-linear-to-r from-indigo-600 to-indigo-700 px-4 py-3 flex items-center justify-between">
              ...
              {editingId === layout.id ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-2 py-1 bg-slate-600 border border-slate-500 rounded-sm text-white text-sm focus:outline-hidden focus:ring-2 focus:ring-blue-500"
                    autoFocus
                  />
                  <button
                    onClick={() => handleRename(layout.id)}
                    className="p-1 text-green-400 hover:bg-green-400/20 rounded-sm"
                  >
                    <Check className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => setEditingId(null)}
                    className="p-1 text-slate-400 hover:bg-slate-600 rounded-sm"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ...
              <div className="flex items-center gap-1">
                {!layout.isDefault && (
                  <button
                    ...
                    className="p-1.5 text-slate-400 hover:text-yellow-400 hover:bg-slate-600 rounded-sm transition-colors"
                ...
                <button
                  ...
                  className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-600 rounded-sm transition-colors"
                  title="Rename"
                >
                ...
                <button
                  ...
                  className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-slate-600 rounded-sm transition-colors"
                  title="Delete"
                >
            ...
            {isDeleting === layout.id && (
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-2 py-1 bg-red-600 text-white text-xs rounded-sm hover:bg-red-500"
                >
                  Yes
                </button>
                <button
                  onClick={() => setIsDeleting(null)}
                  className="px-2 py-1 bg-slate-600 text-white text-xs rounded-sm hover:bg-slate-500"
                >
                  No
                </button>
              </div>
        ...
        {isCreating ? (
          <div className="flex items-center gap-3 px-4 py-3 bg-slate-700/30">
            ...
            <input
              ...
              className="flex-1 px-2 py-1 bg-slate-600 border border-slate-500 rounded-sm text-white text-sm placeholder-slate-400 focus:outline-hidden focus:ring-2 focus:ring-green-500"
              autoFocus
            />
            <button
