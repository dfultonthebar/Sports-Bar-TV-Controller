...
function ResultBadge({ result }: { result: RunResult }) {
  if (!result) {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-xs border bg-slate-500/20 text-slate-400 border-slate-500/30">
        none
      </span>
    )
  }
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-xs border ${RESULT_BADGE[result]}`}
    >
      ...
      <div className="rounded-lg border border-slate-700 p-6">
        <h3 className="text-lg font-semibold text-white flex items-center gap-2 mb-4">
          ...
          {status.currentlyRunning && (
            <span className="ml-2 inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-xs border bg-blue-500/20 text-blue-400 border-blue-500/30">
