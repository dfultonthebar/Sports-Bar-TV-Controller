...
export default function AtlasProgrammingInterface() {
  ...
  const fetchConfiguration = async (processorId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        logger.info('[Atlas Config] Configuration loaded successfully')
      } else {
    ...
    if (updates.gainDb !== undefined && selectedProcessor) {
      try {
        ...
        if (!response.ok) {
          ...
          logger.error('Failed to set input gain:', error)
          showMessage(`Failed to set gain for ${input.name}: ${error.error || 'Unknown error'}`, 'error')
        } else {
          logger.info(`Successfully set gain for Input ${input.physicalInput} to ${updates.gainDb}dB`)
        }
      } catch (error) {
  ...
  const queryHardware = async () => {
    ...
    try {
      ...
      logger.error('Error querying hardware:', error)
  ...
  const addProcessor = async () => {
    try {
      ...
      logger.error('Error adding processor:', error)
    ...
    try {
      ...
      if (result.connected) {
        ...
        if (result.troubleshooting?.steps) {
          logger.info('Troubleshooting steps:', result.troubleshooting.steps.join('\n'))
        }
