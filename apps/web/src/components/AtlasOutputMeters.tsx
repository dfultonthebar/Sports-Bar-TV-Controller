...
export default function AtlasOutputMeters({
  ...
  useEffect(() => {
    ...
    if (autoRefresh) {
      ...
      fetchOutputMeters()
      pollingIntervalRef.current = setInterval(fetchOutputMeters, refreshInterval)
    }
  ...
  }, [processorId, processorIp, autoRefresh, showGroups, refreshInterval])
  ...
  const connectWebSocket = () => {
    try {
      ...
      wsRef.current.onopen = () => {
        logger.info('WebSocket connected for output meter updates')
        setIsConnected(true)
        setError(null)
        setLoading(false)
      }
  ...
  const fetchOutputMeters = async () => {
    try {
      ...
      logger.error('Error fetching output meters:', err)
