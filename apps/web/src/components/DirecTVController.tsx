...
export default function DirecTVController() {
  ...
  const loadDirecTVDevices = async () => {
    try {
      ...
      logger.error('Failed to load DirecTV devices:', error)
    }
  }
  ...
  const loadMatrixInputs = async () => {
    ...
    try {
      ...
      logger.error('Failed to load matrix inputs:', error)
    } finally {
  ...
  const sendDirecTVCommand = async (command: string) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        logger.info('DirecTV command sent:', data.message)
      ...
      logger.error('Failed to send DirecTV command:', error)
  ...
  const addDirecTVDevice = async () => {
    ...
    try {
      ...
      logger.error('Failed to add DirecTV device:', error)
  ...
  const updateDirecTVDevice = async () => {
    ...
    try {
      ...
      logger.error('Failed to update DirecTV device:', error)
  ...
  const deleteDirecTVDevice = async () => {
    ...
    try {
      ...
      logger.error('Failed to delete DirecTV device:', error)
                ...
                <div className="grid grid-cols-1 gap-1 max-h-32 overflow-y-auto">
                  {filteredSportsFavorites.map((favorite) => (
                    <button
                      ...
                      className="text-left p-2 hover:bg-slate-700 rounded-sm text-sm border border-slate-600 transition-colors"
                    >
                      <div className="flex items-center justify-between">
            ...
            <div className="mb-4">
              ...
              <div className="flex justify-center space-x-2 mt-2">
                ...
                <button
                  ...
                  className="bg-gray-500 hover:bg-gray-600 text-white px-3 py-1 rounded-sm text-sm"
                >
                  Exit
                </button>
              </div>
