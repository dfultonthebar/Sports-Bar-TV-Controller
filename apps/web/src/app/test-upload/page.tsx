      ...
      <div className="space-y-4">
        <div>
          ...
          <input
            ...
            className="block w-full text-sm border rounded-sm p-2"
          />
        </div>
        ...
        <button
          ...
          disabled={!file}
          className="px-4 py-2 bg-blue-600 text-white rounded-sm disabled:bg-gray-400"
        >
          Upload
        </button>

        {status && (
          <div className={`p-4 rounded-sm ${status.includes('successful') ? 'bg-green-100' : status.includes('Error') || status.includes('failed') ? 'bg-red-100' : 'bg-yellow-100'}`}>
            {status}
          </div>
        )}

        {result && (
          <pre className="p-4 bg-gray-100 rounded-sm overflow-auto text-sm">
