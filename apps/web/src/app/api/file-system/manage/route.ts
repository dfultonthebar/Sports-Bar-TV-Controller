...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!isAllowedPath(directory)) {
      logger.warn('[FILE-SYSTEM] Blocked path traversal attempt in GET', { directory })
      return NextResponse.json(
        { error: 'Directory is outside allowed paths' },
        { status: 403 }
      )
    }
    ...
    logger.error('[FILE-SYSTEM] GET operation failed', { error: error.message })
    return NextResponse.json({
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!isAllowedPath(targetPath)) {
      logger.warn('[FILE-SYSTEM] Blocked path traversal attempt in POST', { path: targetPath, action })
      return NextResponse.json(
    ...
    logger.error('[FILE-SYSTEM] POST operation failed', { error: error.message })
    return NextResponse.json({
