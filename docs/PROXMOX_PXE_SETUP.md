...
**Workflow:** new NUC arrives at the office → plugs into the LAN → PXE-boots a menu → pick "Sports Bar TV Controller v3.0" → ISO installs + runs the wizard → operator ships the NUC to the bar already-configured.
...
[ Office Router (DHCP) ]
         ...
         │                         ├── /iso/sports-bar-tv-controller-v3.0-*.iso
         ...
         └── [ NEW NUC (PXE-boot enabled in BIOS) ]
              ...
              iPXE loads → fetches sports-bar.ipxe menu → operator picks ISO
                   ↓
              Boots ISO over HTTP → installs to disk → reboot → wizard
```
...
4. **Pick "Install Sports Bar TV Controller v3.0"**.
5. ISO loads → standard install flow → reboot.
6. After reboot: the v2.54.51+ canonical pipeline takes over (first-boot-fresh.sh + location-setup-wizard).
...
| ISO downloads but installer doesn't boot | Memory test failure (NUC has <4 GB RAM) | Standard Ubuntu live ISO needs 2 GB+ RAM; v3.0 ISO is the same |
...
- **Multi-version menu**: pin v3.0 + dev/test build side-by-side so the operator can boot either
...
- **Automated unattended install**: kernel cmdline `sports_bar_mode=fresh auto=true preseed-url=...` would skip the disk-installer prompt and auto-pick disk 0. Useful for batch installs.
