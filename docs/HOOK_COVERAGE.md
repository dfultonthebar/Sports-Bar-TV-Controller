...
| ~~**Phase 4**~~ ✅ **Phase 4 shipped (v2.55.27)** — Auto-grok on critical-path diffs | `scripts/grok-prepush-review.sh` (~165 lines) invoked by `.githooks/pre-push` when the push range touches 13 critical-path globs (schema, drizzle, auto-update, verify-install, iso, proxmox, instrumentation, next.config, ecosystem.config, pre-push hook itself). 120s timeout, per-day SHA cache, auto-injected gotcha hints. Soft block: FINDING requires `--no-verify` or `GROK_PREPUSH_DISABLE=1`. Auto-degrades if `grok` CLI is absent. | Standing Rules 3 + 10 (independent review on the highest-risk changes — schema migrations + auto-update path + ISO build) |
